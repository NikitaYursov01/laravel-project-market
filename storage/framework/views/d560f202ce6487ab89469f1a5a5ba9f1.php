...
<?php $__env->startSection('content'); ?>
  ...
  <div
    ...
    <section
      class="relative flex flex-col items-center justify-center border border-blue-600 border-dashed py-6 md:py-8 px-4 md:px-10 rounded-2xl md:rounded-3xl bg-white w-full max-w-lg mx-4">
      <div class="w-full max-w-md px-2 md:px-0">
        ...
        <div class="bg-white">
          ...
          <!-- ФОРМА ВХОДА ПО КОДУ -->
          <form method="POST" action="<?php echo e(route('login.code')); ?>" id="code_form" class="space-y-4">
            <?php echo csrf_field(); ?>
          ...
          <form method="POST" action="<?php echo e(route('login')); ?>" id="password_form" class="space-y-4 hidden">
            <?php echo csrf_field(); ?>
...
unset($__errorArgs, $__bag); ?>
            ...
            <!-- email -->
            <input id="check_email" name="email" type="email" placeholder="Электронная почта" value="<?php echo e(old('email')); ?>"
  ...
  <script>
    ...
    async function next() {
      ...
      try {
        const response = await fetch('<?php echo e(route("send.code")); ?>', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
...
<?php echo $__env->make('componet.shablon', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Huawei Matebook\Desktop\Проект\laravel-project-market\resources\views/auth/signin.blade.php ENDPATH**/ ?>