...
@section('content')
    ...
    <!-- фон -->
    <div
        class="flex flex-col justify-center items-center w-full min-h-screen bg-[url('/src/background/auth/background/bg.avif')] bg-center bg-cover">
        ...
        <section
            ...
            <div class="w-full">
                ...
                <div class="flex justify-center mb-8">
                    <div
                        class="w-16 h-16 sm:w-20 sm:h-20 bg-blue-600 rounded-full flex justify-center items-center p-3 sm:p-4">
                ...
                <div class="bg-white">
                    ...
                    <form method="POST" action="{{ route('login') }}" id="password_form" class="space-y-4 hidden">
                        ...
                        <!-- email -->
                        <input id="check_email" name="email" type="email" placeholder="Электронная почта"
                            value="{{ old('email') }}"
    ...
    <script>
        ...
        function setBorderState(el, isValid) {
            // Удаляем все цветные классы границ
            el.classList.remove('border-red-500', 'border-green-500', 'border-gray-300', 'border-[#D1D5DB]');
            // Добавляем нужный
            el.classList.add(isValid ? 'border-green-500' : 'border-red-500');
        }
        ...
        async function validateEmail() {
            ...
            if (!emailValue) {//пустая
                ...
                checkpoint_email = 0; //доступ заприщен
            } else if (!(/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(emailValue))) {//некорректный формат
                errorEmail.classList.remove('hidden');//раскрываем ошибку
                setBorderState(email, false);//делаем красным поле
            ...
            next_btn.disabled = checkpoint_email !== 1;
        }

        email.addEventListener('input', validateEmail);

        // Обработчик paste для гарантии валидации после вставки
        ...
        // Глобальное обновление состояния кнопок
        ...
        function validatePasswordEmail() {
            ...
            if (!emailValue) {
                ...
                errorEmailPassword.classList.add('hidden');
                setBorderState(check_email, true);
        ...
        check_email.addEventListener('input', validatePasswordEmail);
        check_email.addEventListener('paste', () => setTimeout(validatePasswordEmail, 0));
        check_email.addEventListener('blur', validatePasswordEmail);
