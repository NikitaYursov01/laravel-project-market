...
@section('content')
    ...
    <div class="max-w-4xl mx-auto px-3 sm:px-4 py-4 sm:py-8">
        <div class="bg-white rounded-lg shadow-md p-4 sm:p-6">
            ...
            <form action="{{ route('orders.store') }}" method="POST" class="space-y-6" enctype="multipart/form-data">
                @csrf
                ...
                <div>
                    ...
                    <select id="category" name="category" required
                        ...
                        <option value="metal_sheet" {{ old('category') == 'metal_sheet' ? 'selected' : '' }}>Листовой металл
                        </option>
                        <option value="metal_pipe" {{ old('category') == 'metal_pipe' ? 'selected' : '' }}>Трубы металлические
                        </option>
                        <option value="metal_beam" {{ old('category') == 'metal_beam' ? 'selected' : '' }}>Балки / Швеллер
                        </option>
                        <option value="metal_rebar" {{ old('category') == 'metal_rebar' ? 'selected' : '' }}>Арматура / Сетка
                        </option>
                        <option value="metal_wire" {{ old('category') == 'metal_wire' ? 'selected' : '' }}>Проволока / Канаты
                        </option>
                        <option value="metal_processing" {{ old('category') == 'metal_processing' ? 'selected' : '' }}>
                            Обработка металла</option>
                        <option value="metal_products" {{ old('category') == 'metal_products' ? 'selected' : '' }}>
                            Металлические изделия</option>
                        <option value="delivery" {{ old('category') == 'delivery' ? 'selected' : '' }}>Доставка / Логистика
                        </option>
                ...
                <div>
                    ...
                    <input type="text" id="budget" name="budget" required
                        ...
                        placeholder="Укажите бюджет" value="{{ old('budget') }}">
                    @error('budget')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea id="materials" name="materials" rows="3"
                        ...
                        placeholder="Укажите, какие материалы предоставите или какие нужны">{{ old('materials') }}</textarea>
                    @error('materials')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="location" name="location"
                        ...
                        placeholder="Город, адрес (если важна геолокация)" value="{{ old('location') }}">
                    @error('location')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea id="technical_requirements" name="technical_requirements" rows="3"
                        ...
                        placeholder="Особые требования, технологии, формат и т.д.">{{ old('technical_requirements') }}</textarea>
